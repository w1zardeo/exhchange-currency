...
const CalendarScreen = ({ navigation }) => {
  ...
  const getCircleStyle = (hasIncompleteTasks, hasCompleteTasks) => {
    if (hasIncompleteTasks) return styles.taskIncomplete(colors);
    if (hasCompleteTasks) return styles.taskComplete(colors);
    return styles.transparent(colors);
  };
      ...
      <View key={day} style={styles.dayContainer}>
        ...
        {isToday(month, day) && (
          <View style={styles.todayContainer}>
            <Text style={[styles.todayText(colors)]}>{t('text.today')}</Text>
          </View>
        )}
      </View>
    );
